...
-- 9 
select m.nome_medico, m.crm, count(c.medico_id) as 'Qauntidade de consultas' from medico m inner join consulta c 
on c.medico_id = m.id_medico group by c.medico_id;


-- 10 
insert into medico(nome_medico, cpf_medico, crm, email_medico, cargo, especialidade_id, em_atividade) values('Renato Alvares', 45679312, 123456, 'alvares.re@gmail.com', 'Especialista', 3, 'Ativo');
insert into endereco(logradouro, cep, bairro, cidade, estado, medico_id, paciente_id) values('R. Ricardo Luis', 04253-020, 'Vila Iara', 'São Paulo', 'São Paulo', 11, null);
insert into telefone(ddd, numero, medico_id, paciente_id) values(11, 943658658, 11, null);

select * from medico where nome_medico like '%Daniel%';
